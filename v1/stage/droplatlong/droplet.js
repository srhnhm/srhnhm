 $(document).ready(function() {
    ...
    function addPin(pin, i, circleData, pace) {
        ...
        var t = pace/50;  //control for overall "seeping in" time
        var stayTime = pace*20;
        var removeTime = pace;
        ...
        function dropBlob (radius, noise, color, strokeColor, stroke, filter, opacInitial, opacFinal, duration) {
            ...
            g.data([circleData]).append("path")
                ...
                .transition().delay(duration+stayTime).duration(removeTime)
                .attr('fill-opacity', 0.00001)
                .transition().delay(duration+stayTime+removeTime+100)
                .remove();
        }
    ...
    function animateStream(pin, i) {
        ...
        $('#stream-'+pin.color).css('left', '+='+pin.newWidth);
    ...
    function prepTweetData(data, colors) {
        ...
        // INCREMENT EXISTING WORDPAIR VERSUS ADD A NEW ONE
            ...
            if (thisTerm) {
                ...
                newTerm.on({
                    click: function(){  
                        var thisBar = $('.tweetBar')[pin.key];
                        console.log(thisBar)
                        $(thisBar).find('ul').html(thisTerm.tweetshtml);
                        $(thisBar).siblings('.tweetBar').css('height', '0em');
                        $(thisBar).css('height', '2em');
                    },
                    mouseenter: function(){
                        $(this).css({
    ...
    var pullTweets = function(query) {
        colorStreams.removeClass('slideLeft');        
        $.getJSON("http://search.twitter.com/search.json"+query+"&callback=?", function(data){
            ...
            colorStreams.addClass('slideLeft');
    ...
    var colors = [
        ...
        {searchterm: 'green', hex: '#759909', terms: {}, offset: 0},
    ...
    var cities = [
        ...
        {key: 'singapore', city: 'Singapore', country: 'Singapore', center: [1.289926,103.860283] },
    ...
    function findCity() {
        var path =  window.location.search.substring().substring(1).toLowerCase()
        var cityObj = cities[5]; // default
        var menu = $('div#citylist').find('ul');
        for (var i=0; i<cities.length; i++) {
            menu.append('<a class="accent" href="'+window.location.pathname+'?'+cities[i].key+'"><li>'+cities[i].city+'</li></a>');
            if (path.match(cities[i].key) != null) {
                cityObj = cities[i];
            }
        }
        $('div#title').html(cityObj.city);
        citylist = $('div#citylist');
        citylist.css('height','auto')
    ...
    var origQuery = "?page=1&geocode="+city.center[0]+"%2C"+city.center[1]+"%2C3mi&rpp=1&q="+colorString(colors)  

    var colorStreams = $('div#colorStream');

    // KICK IT OFF // *************************************************************        
    $.getJSON("http://search.twitter.com/search.json"+origQuery+"&callback=?", function(data) {
        var nextPage = data.next_page.replace('rpp=1', 'rpp=100');
