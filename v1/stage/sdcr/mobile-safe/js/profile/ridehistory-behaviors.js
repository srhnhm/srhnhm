	...
	var cssFile = d3.select('head').append("link").attr({
			"rel": "stylesheet",
			"type": "text/css",
			"href": window.location.href + 'externals/info/ridehistoryfiles/'+viewParameters.view+'.css'
		})
	...
	function drawView(viewParameters) {
		$('#vis').html("<div id='loader'><img src='"+window.location.href+"/images/imgs/loader.gif'></div>")
		var userID = 10147;
		...
		if (viewParameters.view == "fourday") {
				...
				$.ajax({
					...
					success: function(data) {
						...
						if (visibleData.length < 4) {
							...
							$.getScript(window.location.href+"/externals/info/ridehistoryfiles/fourday.js", function() {
			...
			$.ajax({
				...
				success: function(data) {
					...
					$.getScript(window.location.href+'/externals/info/ridehistoryfiles/month.js', function() {
						drawData(visibleData);
					})
				}
			});
			...
			$.ajax({
				...
				success: function(data) {
					visibleData=data.Weeks
					$.getScript(window.location.href+'/externals/info/ridehistoryfiles/list.js', function() {
						drawData(visibleData);
					})
				}
			});
